...
.card-tag { display: inline-flex; width: max-content; padding: 4px 6px; border: 1px solid rgba(255, 90, 30, .48); color: var(--orange-hot); font: 500 8px/1 var(--mono); letter-spacing: .1em; text-transform: uppercase; }
...
.contact-actions { display: flex; justify-content: flex-end; align-items: center; gap: 12px; margin-top: 18px; }
.email-link { display: inline-flex; align-items: center; gap: 12px; padding: 15px 18px; background: var(--black); color: var(--white); font: 800 12px/1 var(--body); letter-spacing: -.01em; transition: background .2s ease, color .2s ease, transform .2s ease; }
.email-link:hover { background: var(--white); color: var(--black); transform: translateY(-3px); }
.media-kit-link { display: inline-flex; align-items: center; min-height: 46px; padding: 12px 14px; border: 1px solid rgba(8, 8, 8, .45); color: var(--black); font: 800 11px/1 var(--body); letter-spacing: -.01em; transition: background .2s ease, color .2s ease, transform .2s ease; }
.media-kit-link:hover { background: var(--black); color: var(--white); transform: translateY(-3px); }
...
/* Brand partnerships */
.brands-hero { position: relative; min-height: 80svh; isolation: isolate; overflow: clip; background: var(--black); }
.brands-hero::after { content: ""; position: absolute; right: -10vw; bottom: -28vw; z-index: -1; width: min(65vw, 880px); aspect-ratio: 1; border-radius: 50%; border: clamp(38px, 7vw, 104px) solid rgba(255, 90, 30, .16); box-shadow: 0 0 130px rgba(255, 90, 30, .12); }
.brands-hero-inner { position: relative; display: grid; align-items: end; min-height: calc(80svh - 86px); padding: clamp(88px, 11vw, 150px) 0 74px; }
.brands-hero h1 { max-width: 950px; margin-top: 25px; font-size: clamp(92px, 13vw, 188px); }
.brands-hero h1 span { color: var(--orange); }
.brands-lede { max-width: 570px; margin-top: 32px; color: var(--white-dim); font-size: clamp(16px, 1.55vw, 20px); line-height: 1.65; }
.brands-intro { padding: clamp(78px, 10vw, 130px) 0; background: var(--white); color: var(--black); }
.brands-intro-grid { display: grid; grid-template-columns: minmax(0, .9fr) minmax(360px, 1.1fr); gap: clamp(38px, 8vw, 124px); align-items: start; }
.brands-intro h2, .brands-section-title { margin-top: 16px; font: 400 clamp(58px, 7.4vw, 102px)/.8 var(--display); letter-spacing: -.025em; text-transform: uppercase; }
.brands-intro .eyebrow { color: var(--orange); }
.brands-deck { max-width: 590px; font-size: clamp(20px, 2.25vw, 31px); font-weight: 600; letter-spacing: -.04em; line-height: 1.22; }
.brands-detail { max-width: 520px; margin-top: 24px; color: #534d45; font-size: 15px; line-height: 1.7; }
.brand-services { padding: clamp(78px, 10vw, 130px) 0; background: var(--black-soft); }
.brand-section-head { display: flex; align-items: end; justify-content: space-between; gap: 30px; }
.brand-section-head p { max-width: 270px; color: var(--white-dim); font-size: 14px; line-height: 1.6; }
.brand-service-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 22px; margin-top: 46px; }
.brand-service { min-height: 280px; display: flex; flex-direction: column; justify-content: space-between; padding: 22px; border: 1px solid var(--line); background: rgba(255, 255, 255, .025); transition: border-color .25s ease, background .25s ease, transform .25s ease; }
.brand-service:hover { border-color: var(--orange); background: #16130f; transform: translateY(-6px); }
.brand-service-number { color: var(--orange-hot); font: 500 10px/1 var(--mono); letter-spacing: .1em; }
.brand-service h3 { max-width: 250px; font: 400 clamp(35px, 3.6vw, 50px)/.86 var(--display); letter-spacing: -.015em; text-transform: uppercase; }
.brand-service p { max-width: 275px; color: var(--white-dim); font-size: 14px; line-height: 1.58; }
.brand-proof { position: relative; overflow: hidden; padding: clamp(78px, 10vw, 130px) 0; background: var(--orange); color: var(--black); }
.brand-proof::after { content: ""; position: absolute; right: -10vw; bottom: -20vw; width: min(55vw, 680px); aspect-ratio: 1; border: clamp(36px, 6vw, 90px) solid rgba(247, 244, 238, .32); border-radius: 50%; }
.brand-proof .eyebrow { color: var(--black); }
.brand-proof-grid { position: relative; z-index: 1; display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px; margin-top: 46px; }
.brand-proof-card { min-height: 210px; padding: 23px; border: 1px solid rgba(8, 8, 8, .25); background: rgba(247, 244, 238, .12); }
.brand-proof-card h3 { font: 400 clamp(37px, 4vw, 55px)/.88 var(--display); letter-spacing: -.015em; text-transform: uppercase; }
.brand-proof-card p { max-width: 370px; margin-top: 19px; color: #3c190d; font-size: 14px; line-height: 1.6; }
.brand-work-links { display: flex; flex-wrap: wrap; gap: 10px; margin-top: 20px; }
.brand-work-links a { display: inline-flex; padding: 8px 10px; border: 1px solid rgba(8, 8, 8, .42); color: var(--black); font: 500 10px/1 var(--mono); letter-spacing: .06em; text-transform: uppercase; transition: background .2s ease, color .2s ease; }
.brand-work-links a:hover { background: var(--black); color: var(--white); }
.brand-cta { padding: clamp(78px, 10vw, 130px) 0; background: var(--black); }
.brand-cta-inner { display: flex; align-items: end; justify-content: space-between; gap: 36px; }
.brand-cta h2 { max-width: 700px; font: 400 clamp(68px, 9vw, 128px)/.78 var(--display); letter-spacing: -.03em; text-transform: uppercase; }
.brand-cta h2 span { color: var(--orange); }
.brand-cta-copy { max-width: 350px; }
.brand-cta-copy p { color: var(--white-dim); font-size: 15px; line-height: 1.62; }
.brand-cta-copy .email-link { margin-top: 23px; }
...
@media (max-width: 800px) {
  ...
  .brands-hero-inner { min-height: auto; padding: 72px 0 76px; }
  .brands-hero h1 { font-size: clamp(76px, 21vw, 130px); }
  .brands-intro-grid, .brand-service-grid, .brand-proof-grid { grid-template-columns: 1fr; }
  .brands-intro h2, .brands-section-title { font-size: clamp(58px, 17vw, 92px); }
  .brand-section-head, .brand-cta-inner { display: block; }
  .brand-section-head p, .brand-cta-copy { margin-top: 24px; }
  .brand-service { min-height: 220px; }
}
@media (max-width: 430px) {
  ...
  .email-link, .media-kit-link { width: 100%; justify-content: space-between; }
  .brands-hero-inner { padding: 54px 0 62px; }
  .brands-lede { margin-top: 23px; }
  .brand-service { min-height: 204px; }
  .brand-proof-card { min-height: 0; }
}
...
def social_card(social):
    tag = f'<span class="card-tag">{escape(social["tag"])}</span>' if social.get("tag") else ""
    return f"""
      <a class="social-card" data-tilt href="{social['url']}" target="_blank" rel="noopener noreferrer" aria-label="Open Roam Prints Studio on {social['name']} (opens in a new tab)">
        ...
        <span class="card-body"><span class="social-name">{social['name']}</span><span class="social-handle">{social['handle']}</span>{tag}</span>
        <span class="card-bottom"><span class="card-arrow" aria-hidden="true">↗</span></span>
      </a>"""


def document(title, description, body, path="/"):
...
def build_home():
  ...
  <main>
    ...
    <section class="contact" id="contact"><div class="shell contact-grid" data-reveal>
      ...
      <div class="contact-copy"><p>For collaborations and brand partnerships</p><div class="contact-actions"><a class="email-link" href="mailto:{EMAIL}" aria-label="Email Roam Prints Studio to work together">Email us to work together <span aria-hidden="true">↗</span></a><a class="media-kit-link" href="brands.html">View media kit <span aria-hidden="true">↗</span></a></div></div>
...
def build_brands():
    body = f"""<body>
  <div id="scroll-progress" aria-hidden="true"></div>
  <a class="skip-link" href="#ways-to-work">Skip to partnership options</a>
  <header class="brands-hero">
    <canvas id="filament-field" aria-hidden="true"></canvas>
    <nav class="shell nav" aria-label="Main navigation">
      <a class="brand" href="/" aria-label="Roam Prints Studio home"><img src="assets/rp-mark-light.png" alt="">ROAM PRINTS / STUDIO</a>
      <div class="nav-links"><a class="nav-pill" href="/">Follow us ↗</a></div>
    </nav>
    <div class="shell brands-hero-inner">
      <div>
        <span class="eyebrow">Brand partnerships</span>
        <h1>Let’s make<br><span>something<br>shareable.</span></h1>
        <p class="brands-lede">Roam Prints creates maker-first content for the tools, ideas, and brands people want to talk about.</p>
      </div>
    </div>
  </header>
  <main>
    <section class="brands-intro"><div class="shell brands-intro-grid" data-reveal>
      <div><span class="eyebrow">The fit</span><h2>Made for<br>maker culture.</h2></div>
      <div><p class="brands-deck">We make short-form content that puts the build, the personality, and the product in the same frame.</p><p class="brands-detail">Our work is a natural fit for 3D printing, creative tools, workshop gear, and brands that want to show up in maker culture with a point of view.</p></div>
    </div></section>
    <section class="brand-services" id="ways-to-work"><div class="shell">
      <div class="brand-section-head" data-reveal><div><span class="eyebrow">Ways to work together</span><h2 class="brands-section-title">Bring us<br>an idea.</h2></div><p>We shape each collaboration around the audience, the product, and a strong reason to stop scrolling.</p></div>
      <div class="brand-service-grid">
        <article class="brand-service" data-reveal><span class="brand-service-number">01 / PRODUCT STORY</span><h3>Product integrations.</h3><p>Bring your tool, filament, accessory, or idea into a story that feels native to the feed.</p></article>
        <article class="brand-service" data-reveal><span class="brand-service-number">02 / SHORT-FORM</span><h3>Original video concepts.</h3><p>Creator-led vertical videos made to earn attention, spark comments, and be worth sharing.</p></article>
        <article class="brand-service" data-reveal><span class="brand-service-number">03 / CUSTOM ART</span><h3>Creative collaborations.</h3><p>Custom-art opportunities through Art by Roam for brands that want something more distinctive.</p></article>
      </div>
    </div></section>
    <section class="brand-proof"><div class="shell">
      <span class="eyebrow">The details that matter</span><h2 class="brands-section-title">Current, not canned.</h2>
      <div class="brand-proof-grid">
        <article class="brand-proof-card" data-reveal><h3>Audience + performance.</h3><p>Current audience snapshots, platform performance, and campaign-fit details are shared for relevant partnership conversations.</p></article>
        <article class="brand-proof-card" data-reveal><h3>See the work.</h3><p>Explore the current content across our channels and get a feel for the tone, concepts, and community.</p><div class="brand-work-links"><a href="https://www.tiktok.com/@roamprints" target="_blank" rel="noopener noreferrer">TikTok ↗</a><a href="https://www.instagram.com/roamprintsstudio" target="_blank" rel="noopener noreferrer">Instagram ↗</a><a href="https://www.youtube.com/@roamprints" target="_blank" rel="noopener noreferrer">YouTube ↗</a></div></article>
      </div>
    </div></section>
    <section class="brand-cta"><div class="shell brand-cta-inner" data-reveal>
      <h2>Let’s make<br><span>something move.</span></h2>
      <div class="brand-cta-copy"><p>Tell us what you’re building. We’ll share the right audience details, recent examples, and a partnership approach tailored to the opportunity.</p><a class="email-link" href="mailto:{EMAIL}?subject=Roam%20Prints%20media%20kit%20request">Request the media kit <span aria-hidden="true">↗</span></a></div>
    </div></section>
    <footer class="contact-foot"><div class="shell contact-foot-inner"><span>© {date.today().year} Roam Prints Studio</span><a href="/">Back to social links ↑</a></div></footer>
  </main>
{SCRIPT}
</body>"""
    return document(
        "Roam Prints Studio — Brand Partnerships",
        "Partner with Roam Prints Studio for creator-led short-form video, product storytelling, and custom-art collaborations.",
        body,
        "/brands.html",
    )
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>{SITE_URL}/brands.html</loc><lastmod>{today}</lastmod><changefreq>monthly</changefreq><priority>0.8</priority></url>
</urlset>
"""
...
write_file("brands.html", build_brands())
